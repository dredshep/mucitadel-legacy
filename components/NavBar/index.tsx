  ...
  return (
    <div
      className={
        ...
        " flex flex-row items-center w-48 mr-auto lg:mx-0"
      }
    >
      {/* ^ justify-between */}
      <MainLogo />
...
function NavRightSide() {
  ...
  const onClickConnect = async () => {
    try {
      const newAccounts = await ((window as unknown) as Window).ethereum.request(
        {
  ...
  return (
    <div className="text-2xl ml-auto lg:ml-0 lg:text-3xl flex space-x-6 items-center">
      ...
      <FontAwesomeIcon
        icon={faPlusSquare}
        className={`hidden${isLoggedIn ? " lg:flex" : ""}`}
      />
      ...
      <div
        ...
        className={isLoggedIn ? "flex flex-column" : "hidden"}
      >
        <FontAwesomeIcon icon={faUserCircle} className="cursor-pointer" />
        <div
          className={
            (popdownIsVisible ? "absolute" : "hidden") +
            " origin-top-right top-14 right-4 bg-asidebg shadow-2xl text-lg rounded-lg flex flex-col font-semibold"
          }
        >
